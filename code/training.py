...
def train(augmented1, augmented2, model, loss_fn, optimizer, text_encoder, image_encoder, clip_zsl, device):
    ...
    for i, (images, label) in enumerate(augmented1): #NOTE: We won't have the labels
        ...
        with torch.no_grad():
            ...
            multiplier = torch.eq(predict_class_first, predict_class_second)

            predict_class_first, predict_class_second = torch.dot(predict_class_first, multiplier)
            augmented1 = torch.dot(augmented1, multiplier)
            augmented2 = torch.dot(augmented2, multiplier)
...
def main():
    ...
    for epoch in range(10):
        ...
        """ predictions1 = self.predict_class(augmented1, initial_embeddings, clip_zsl)
        predictions2 = self.predict_class(augmented2, initial_embeddings, clip_zsl) """
